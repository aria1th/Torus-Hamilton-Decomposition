...
\paragraph{Color $2$.}
The derivation for color $2$ follows the same arithmetic-trace schema as Table~\ref{tab:c0_case1}.
On each region of the Case~I partition the layer-$1$ and layer-$2$ tests produce the low-layer word $\omega_2$; the word-to-displacement dictionary then yields the Case~I formula for $R_2$ displayed in Appendix~\ref{app:routeE}.
...
\paragraph{Color $0$.}
The same arithmetic-trace procedure applied to the Case~II partition produces the low-layer words $\omega_0$ for each region.
The word-to-displacement dictionary is identical to Case~I; evaluating it on the Case~II partition verifies the displayed Case~II formula for $R_0$.

\paragraph{Color $2$.}
The analogous evaluation for color $2$ over the Case~II partition follows the same schema as Table~\ref{tab:c0_case1}.
Grouping the resulting words by displacement matches the uniform formula for $R_2$ displayed in Appendix~\ref{app:routeE}.
...
\end{enumerate}
For larger even $m$, the bundle also includes \texttt{routee\_large\_m\_certificate.py}, which assumes the proved splice normal forms and return-time identities and evaluates the resulting Hamiltonicity certificate in constant time per tested value of $m$.
This optimized path is intended for post-proof stress tests at very large even $m$.
The main script \texttt{route\_e\_even.py} still provides the formula-driven regression checks by explicitly summing the closed-form first-return data, which is useful when one wants an independent audit of the displayed formulas on moderate ranges.
These computations are not part of the proofs in the present paper; they serve as verification aids for the even-case construction and as regression checks for the displayed closed-form formulas.
