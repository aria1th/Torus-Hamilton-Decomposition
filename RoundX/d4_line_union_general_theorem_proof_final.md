...
- if \(S(x)\ge 3\): use the canonical tuple \((0,1,2,3)\).
...
- For colors \(0,1\), after two steps the layer-2 landing point is
  ...
  \]
  Here the even gate condition is \(x_3=0\iff b=1\), and the odd gate condition is \(x_0=0\iff a=m-1\).
...
- For colors \(2,3\), after two steps the layer-2 landing point is
  ...
  \]
  Here the even gate condition is \(x_3=0\iff b=2\), and the odd gate condition is \(x_0=0\iff a=0\).
...
\]
Since each \(R_c\) changes \(q\) by \(+1\) or \(-1\) at every step, every \(R_c\)-orbit meets \(Q\) once every \(m\) iterates. Define the second return
\[
T_c=R_c^m\big|_Q.
\]
...
Conversely, if \(O^t(u,v)=(u,v)\), write \(t=qm+r\) with \(0\le r<m\). The first coordinate changes by \(r\), so necessarily \(r=0\). Thus \(t=qm\), and applying \(O^m(u,v)=(u,v+1)\) repeatedly gives
\[
O^t(u,v)=(u,v+q).
\]
Hence \(q\equiv 0\pmod m\), so \(t\) is a multiple of \(m^2\). Thus every orbit has exact length \(m^2\), and since the state space has size \(m^2\), there is only one orbit. ∎
...
### Proof
Under every \(R_c\), the \(q\)-coordinate changes by \(+1\) or \(-1\), depending only on the color. Hence each \(R_c\)-orbit in \(P_0\) meets \(Q\) exactly once every \(m\) steps. Equivalently, every point of \(P_0\) can be written uniquely as
\[
...
\]
These are pairwise distinct because their \(q\)-values determine \(j\) uniquely modulo \(m\). Hence the orbit has size \(m\cdot m^2=m^3=|P_0|\), so \(R_c\) is a single cycle on \(P_0\). ∎
...
### Proof
Every application of \(f_c\) increases \(S\) by \(1\), so each \(f_c\)-orbit meets \(P_0\) exactly once every \(m\) steps. Thus every vertex is uniquely of the form
\[
f_c^j(x),\qquad x\in P_0,\quad 0\le j<m.
\]
If \(R_c\) is one cycle on \(P_0\), then the \(f_c\)-orbit of one point in \(P_0\) contains all such vertices, hence all \(m\cdot m^3=m^4\) vertices of the torus. ∎
