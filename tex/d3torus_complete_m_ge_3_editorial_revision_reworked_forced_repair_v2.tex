...
For even $m$, the parity obstruction rules out all Kempe-from-canonical approaches, and the proof relies on an explicit direction assignment that breaks the canonical pattern on the layers $S\in\{0,1,2\}$.
The even argument proceeds as a deductive surgery: a natural primary geometry already works for colors $0$ and $2$, but fails first in color $1$ when $m\equiv 4\pmod 6$.
The remaining Case~II boundary corrections alone do not repair that failure; the added affine defect family changes the interior first-return shifts before a finite splice closes the cycles.
...
Taken together, Proposition~\ref{prop:primary-geometry-obstruction} and Proposition~\ref{prop:boundary-no-go} show how the even proof should be read: primary geometry, precise obstruction, failure of the remaining boundary corrections alone, and then the actual bulk repair.
...
Proposition~\ref{prop:boundary-no-go} then shows that deleting the added affine family while keeping the remaining Case~II special points already makes the induced lane maps noninjective.
So in the actual Route~E scaffold the Case~II family is genuine bulk repair, not a cosmetic endpoint adjustment.
...
The Route~E construction shares the odd-case return-map framework but adds an explanatory internal structure: the primary geometry closes all colors except color~$1$ when $m\equiv 4\pmod 6$, and the added Case~II affine family provides genuine bulk repair (Proposition~\ref{prop:boundary-no-go}).
Once the repaired first-return formulas are known, the closure step reduces to a splice permutation on arithmetic family-blocks (Proposition~\ref{prop:routeE-splice}), while Lemma~\ref{lem:counting} upgrades the single-cycle property to full Hamiltonicity.
\end{remark}
...
The main text proves color~$2$ in full and also isolates the obstruction/repair narrative for Case~II: primary geometry, residue-$3$ obstruction in color~$1$, and the noninjectivity no-go obtained by deleting the added family while retaining the remaining Case~II corrections.
This appendix records the remaining first-return computations for the actual repaired geometry in colors~$1$ and~$0$; they are the detailed verification of the defect itineraries, lane maps, and return-time sums once the conceptual architecture from the main text has been fixed.
