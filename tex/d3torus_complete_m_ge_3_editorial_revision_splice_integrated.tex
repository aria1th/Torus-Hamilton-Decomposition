...
Layer $0$ carries the main splicing burden: its casework, which depends on $m\bmod 6$, cuts the bulk arithmetic runs at finitely many bridge points and splices the resulting arithmetic family-blocks into a single cyclic order.
...
The boundary-point exceptions are then tuned only to cut the long arithmetic runs---maximal runs of consecutive lanes that share the same bulk itinerary---at finitely many bridge vertices and splice the resulting ordered arithmetic family-blocks into a single cyclic order.
In particular, once the first-return formulas are known, the single-cycle question for the induced lane map reduces to the cycle structure of a finite splice permutation on those arithmetic family-blocks.
The residue-class split $m\equiv 0,2\pmod{6}$ versus $m\equiv 4\pmod{6}$ appears only at this final splicing stage: for $m\equiv 0,2\pmod{6}$ the primary defect lines already connect the relevant arithmetic runs, whereas for $m\equiv 4\pmod{6}$ one additional affine splice family is needed for colors $1$ and $0$.
\end{remark}
...
For colors $1$ and $0$, Appendix~\ref{app:routeE-cycle} follows the same pattern.
Propositions~\ref{prop:R1-caseI}, \ref{prop:R1-caseII}, \ref{prop:R0-caseI-data}, and \ref{prop:R0-caseII-data} give the explicit formulas for the induced lane maps $T_c$ and the return times $\rho_c$.
Proposition~\ref{prop:routeE-splice} then packages the single-cycle step for the induced lane maps as a finite splice permutation on arithmetic family-blocks: the bulk rule advances inside each block, and the defect set determines only the induced permutation on the block labels.
...
Where the odd case enjoys a single uniform return-map formula, the even return maps have piecewise definitions with an $m\bmod 6$ split and finitely many boundary-point corrections.
Its low-layer modifications are supported on a bounded number of affine defect lines and isolated points inside the planes $S=0,1,2$.
At the level of the induced lane maps, however, the final closure step has a simple normal form: away from the defect set, the orbit follows the bulk rule inside long arithmetic runs, and the defects only cut and splice finitely many arithmetic family-blocks.
...
First, the present proof now isolates the final Route~E closure step as a splice-graph normal form on arithmetic family-blocks: the low-layer defects do not create new long-range dynamics inside the bulk arithmetic runs, but only cut and splice finitely many such runs.
What remains open is whether one can read off the resulting splice permutation directly from the affine defect geometry, without the itinerary bookkeeping used here, and whether this viewpoint can explain conceptually why the mod-$6$ split is unavoidable.
