...
a return-map and odometer proof for all $m\ge 3$}
...
The proof is organized around return maps rather than around the full $m^3$-vertex dynamics.
Every legal step increases the layer function $S=i+j+k$ by $1$, so Hamiltonicity reduces to the $m$-step return maps on the plane $P_0=\{S=0\}$ of size $m^2$.
The model return map is the standard two-dimensional odometer
\[
O(u,v)=(u+1,\ v+\mathbf 1_{u=0}),
\]
whose first coordinate is the clock and whose second coordinate records the carry.
For odd $m$, five Kempe swaps applied to the canonical coloring produce a valid coloring whose three return maps on $P_0$ are explicitly affine-conjugate to $O$.
For even $m$, a sign-product invariant shows that no Kempe-from-canonical construction can work.
We therefore build an explicit low-layer direction assignment, Route~E, supported on the three lowest layers.
In adapted bulk coordinates its return maps are finite-defect perturbations of the same clock-and-carry mechanism; after one further first-return reduction to lane transversals, only a finite splice analysis remains.
The boundary case $m=4$ is handled by an explicit finite witness.

Thus the paper gives a complete Hamilton decomposition theorem for $D(m)$ together with a proof template in which odd $m$ is an exact affine odometer theorem and even $m$ is a repaired clock-and-carry theorem.
\end{abstract}
...
In the odd case, passing to the section $P_0$ already reveals the full two-dimensional clock-and-carry map, and the proof closes by explicit affine conjugacy to the standard odometer.
In the even case, the parity barrier shows that the canonical Kempe route cannot succeed, so the construction has to rebuild that mechanism directly.
...
The boundary case $m=4$ is the finite exceptional instance in which the same low-layer surgery is recorded by a table rather than by a uniform arithmetic family.
The accompanying scripts, bundled as supplementary material in \texttt{anc/}, are independent regression checks and do not form part of the proof.
