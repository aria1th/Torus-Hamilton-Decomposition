#!/usr/bin/env python3
"""Validate reported survivors from the shared-predecessor interaction search."""
    ...
    *,
    ...
    for row in candidate_rows:
        ...
        matches_search = (
            ...
            and pilot_eval["universal_m_cycle"] == row["search_metrics"]["universal_m_cycle"]
        )
        rows.append(
            {
                "label": row["label"],
...
def _print_summary(summary: Dict[str, object], *, use_rich: bool) -> None:
    ...
    for row in summary["candidates"][:8]:
        ...
        lines.append(
            f"{row['label']} stage={r['stage']} flag={r['predecessor_flag_name']} "
            f"l2=({r['layer2_mode_p0']['name']},{r['layer2_mode_p1']['name']}) "
            f"l3={r['layer3_bit_name']} ({r['layer3_mode_p0']['name']},{r['layer3_mode_p1']['name']}) "
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Validate the d=5 shared-predecessor interaction family.")
