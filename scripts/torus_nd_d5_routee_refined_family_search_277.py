#!/usr/bin/env python3
"""Search a refined Route-E assembly family guided by the d5_276 defect markers.

This refines the d5_274 class family by adding two small local bits:

- g_anchor(x) = 1 iff x2 = m-1 and x0 + 2*x1 + x4 == 1 (mod m)
- m_bad(x)    = 1 iff x4 = m-1, x2 != m-1, and 2*x0 + 3*x1 == 1 (mod 3)

The motivation is d5_276: the residual color-2 short cycles in the combined
best d5_274 family are organized by a universal G-only family and an extra
M-only family on 3|m.  This script asks whether allowing the exactness CSP to
see those two markers already yields a significantly better five-color family.

Outputs:
- RoundY/checks/d5_277_routee_refined_family_search_summary.json
- RoundY/checks/d5_277_routee_refined_family_search/search.json
"""
...
DEFAULT_SUMMARY = REPO_ROOT / "RoundY" / "checks" / "d5_277_routee_refined_family_search_summary.json"
DEFAULT_DETAIL_DIR = REPO_ROOT / "RoundY" / "checks" / "d5_277_routee_refined_family_search"
...
def m_bad(x: tuple[int, int, int, int, int]) -> int:
    return int(x[4] != x[2] and x[4] >= 0 and x[2] >= 0 and (2 * x[0] + 3 * x[1]) % 3 == 1)
...
def family_key(m: int, x: tuple[int, int, int, int, int]) -> tuple[object, ...]:
    ...
    if sigma == 0:
        ...
        base = ("S4+",)
    # The Route-E refinement only needs two tiny local markers.
    return base + (g_anchor(m, x), int(x[4] == m - 1 and x[2] != m - 1 and (2 * x[0] + 3 * x[1]) % 3 == 1))
...
def class_name(key: tuple[object, ...]) -> str:
    ...
    if base == ("S0",):
        ...
        label = f"S2({{{subset}}})"
    return f"{label}|G{g_bit}|M{m_bit}"
...
def build_model(
    ...
    model.Add(vars_by_key[key_to_idx[("S0", 0, 0)]] == PERM_INDEX[(0, 1, 2)])
    for row in patterns:
...
def hamilton_colors_from_histograms(m: int, histograms: dict[str, dict[str, int]]) -> list[int]:
    ...
    out = [3, 4]
    for color in (0, 1, 2):
        if histograms[str(color)] == {full: 1}:
            out.append(color)
    return sorted(out)
...
class SearchCallback(cp_model.CpSolverSolutionCallback):
    ...
    def on_solution_callback(self) -> None:
        ...
        score = (min(ham_counts.values()), sum(ham_counts.values()), color2_max, color1_max)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Search a Route-E refined family guided by d5_276 defect markers.")
    parser.add_argument("--summary-json", type=Path, default=DEFAULT_SUMMARY)
    parser.add_argument("--detail-dir", type=Path, default=DEFAULT_DETAIL_DIR)
    parser.add_argument("--search-moduli", nargs="*", type=int, default=[7, 9])
    parser.add_argument("--evaluation-moduli", nargs="*", type=int, default=[5, 7, 9])
    ...
    if cb.best_payload is None or cb.best_score is None:
        raise RuntimeError("refined search produced no solution")

    summary = {
        "task": "d5_277_routee_refined_family_search",
        "family": (
            "Refine d5_274 by adding the two d5_276 defect markers "
            "g_anchor = 1_{x2=m-1 and x0+2x1+x4=1 mod m} and "
            "m_bad = 1_{x4=m-1, x2!=m-1, 2x0+3x1=1 mod 3}."
        ),
        ...
        "conclusion": (
            "The d5_276 defect markers can be injected directly into the finite exactness CSP. "
            "The resulting search tests whether the residual five-color assembly obstruction is "
            "already resolved by a tiny Route-E-style refinement of the d5_274 class family."
        ),
    }
