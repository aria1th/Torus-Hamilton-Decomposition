#!/usr/bin/env python3
"""Search the d=5 synchronized wu2 exact-bit family."""
...
from torus_nd_d5_synchronized_wu2_exact_common import (
...
def _serialize_row(result: Mapping[str, object]) -> Dict[str, object]:
    ...
    return {
        "rule": result["rule"],
        "layer3_dependency_class": layer3_dependency_class(rule),
        "synchronized_dependency_class": synchronized_dependency_class(rule),
...
def _dependency_counts(rows: Sequence[Dict[str, object]], key: str) -> Dict[str, int]:
    counter = Counter()
    for row in rows:
        if row["clean_all"] and row["strict_all"]:
        ...
        out.append(
            {
                "group": {
                    ...
                    "sync_flip_pattern_name": key[3],
                },
                "counts": _counts(items),
                "layer3_dependency_class_counts": _dependency_counts(items, "layer3_dependency_class"),
                "synchronized_dependency_class_counts": _dependency_counts(items, "synchronized_dependency_class"),
...
def run_search(*, m_values: Sequence[int], mixed_baseline_json: Path | None, cycle_baseline_json: Path | None) -> Dict[str, object]:
    ...
    summary = {
        ...
        "stage1": {
            ...
            "layer3_dependency_class_counts": _dependency_counts(stage1_rows, "layer3_dependency_class"),
            "synchronized_dependency_class_counts": _dependency_counts(stage1_rows, "synchronized_dependency_class"),
    ...
    if stage2_rows:
        summary["stage2"] = {
            ...
            "layer3_dependency_class_counts": _dependency_counts(stage2_rows, "layer3_dependency_class"),
            "synchronized_dependency_class_counts": _dependency_counts(stage2_rows, "synchronized_dependency_class"),
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Search the d=5 synchronized wu2 exact-bit family.")
