#!/usr/bin/env python3
"""Common evaluator for the d=5 wu2-secondary exact-twist search."""
...
LAYER2_BASES = OrderedDict(
    [
        ("const_2/4", {"mode_name": "w+u=2:2/4"}),
        ("const_4/2", {"mode_name": "w+u=2:4/2"}),
    ]
)
...
class Rule:
    ...
    def payload(self) -> Dict[str, object]:
        mapping = {}
        for label, name in self.controller_mode_map().items():
            s0, s1 = SLICE_MODES[name]
            mapping[label] = {"name": name, "table": {"s0": s0, "s1": s1}}
        return {
            "stage": self.stage,
    ...
    def from_payload(cls, payload: Mapping[str, object]) -> "Rule":
        modes = payload["controller_modes"]
        return cls(
...
def _feature_tuple(coords: Sequence[int], color: int, m: int) -> Tuple[int, int, int, int, int, int]:
...
def anchor_by_feature(pre: Mapping[str, object], rule: Rule) -> List[int]:
    ...
    base_mode = ALT4_MODE_TABLES[LAYER2_BASES[rule.layer2_base_name]["mode_name"]]
    controller_bit_index = 2 if rule.controller_flag_name == "pred_sig1_wu2" else 3
    mapping = rule.controller_mode_map()
    for feature_id, feature in enumerate(pre["feature_rows"]):
        ...
        if layer_code == 4:
            ...
            continue
        if layer_code == 2:
            anchors[feature_id] = base_mode["s1"] if wu2 else base_mode["s0"]
            continue
        r = (flag_mask >> controller_bit_index) & 1
        p = exact_p
        mode_name = mapping[f"{r}{p}"]
