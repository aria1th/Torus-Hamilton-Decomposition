#!/usr/bin/env python3
"""Validate reported survivors from the alt-4 two-flag geometry search."""
...
def _print_summary(summary: Dict[str, object], *, use_rich: bool) -> None:
    ...
    for row in summary["candidates"][:8]:
        r = row["rule"]["layer2_state_modes"]
        v = row["pilot_validation"]
        lines.append(
            f"{row['label']} states=({r['00']['name']},{r['01']['name']},{r['10']['name']},{r['11']['name']}) "
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Validate the d=5 alt-4 two-flag geometry family.")
