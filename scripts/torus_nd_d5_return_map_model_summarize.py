#!/usr/bin/env python3
"""Summarize saved d=5 return-map tables into a compact model report."""
...
def _r_state_list(table: Mapping[str, object]) -> Sequence[Tuple[int, int, int]]:
    return [(row["q"], row["w"], row["u"]) for row in table["states"].values() if row["clean_state"]]


def _u_state_list(table: Mapping[str, object]) -> Sequence[Tuple[int, int]]:
    return [(row["w"], row["u"]) for row in table["states"].values()]


def _analyse_tables(name: str, m: int, tables_dir: Path) -> Dict[str, object]:
    first_path = tables_dir / f"{name}_m{m}_first_return.json"
    grouped_path = tables_dir / f"{name}_m{m}_grouped_return.json"
    quotient_r_path = tables_dir / f"{name}_m{m}_quotient_R.json"
    quotient_u_path = tables_dir / f"{name}_m{m}_quotient_U.json"

    first_return = _load_json(first_path)
    grouped_return = _load_json(grouped_path)
    quotient_r = _load_json(quotient_r_path)
    quotient_u = _load_json(quotient_u_path)

    r_increment_models = {}
    r_affine = {}
    if first_return["clean_frame"] and first_return["strict_clock"] and m in PILOT_M_VALUES:
    ...
    elif first_return["clean_frame"] and first_return["strict_clock"]:
        r_increment_models = {"skipped": "stability_only"}
        r_affine = {"skipped": "stability_only"}
...
def run_summary(*, m_values: Sequence[int], tables_dir: Path) -> Dict[str, object]:
    start = __import__("time").perf_counter()
    specs = load_witness_specs()
    witnesses = []
    for spec in specs:
        reports = [_analyse_tables(spec.name, m, tables_dir) for m in m_values]
        witnesses.append(
            {
                "witness": {
                    "name": spec.name,
                    ...
                    "family": spec.family,
                    "source": spec.source,
                    "rule": spec.rule_payload,
                },
                "reports_by_m": reports,
            }
        )
    return {
        "task_id": TASK_ID,
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Summarize saved d=5 return-map model tables.")
    parser.add_argument("--m-list", default="5,7,9,11,13", help="comma-separated moduli to summarize")
    parser.add_argument("--tables-dir", type=Path, required=True, help="directory containing saved table JSON files")
    parser.add_argument("--summary-out", type=Path, required=True, help="write summary JSON here")
    parser.add_argument("--no-rich", action="store_true", help="disable Rich output")
    args = parser.parse_args(argv)

    summary = run_summary(m_values=parse_args_m_list(args.m_list), tables_dir=args.tables_dir)
