#!/usr/bin/env python3
"""Free-anchor search for d=5 cyclic-equivariant master fields on the joined quotient Theta_AB."""
    ...
    for item in summary["schema_results"]:
        ...
        if item.get("feasible_fields"):
            ...
            line += (
                ...
                f"strict_clock={first['color0_return']['strict_clock']} "
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Search free-anchor d=5 master fields on the joined Theta_AB quotient.")
    parser.add_argument("--schemas", default="joined_anchor_five_atom", help="comma-separated schema names")
    ...
    parser.add_argument("--solution-limit", type=int, default=10, help="max solutions to save from each enumeration pass")
    parser.add_argument("--out", type=Path, required=True, help="write JSON summary to this path")
    parser.add_argument("--state-table-out", type=Path, help="write joined quotient state table JSON")
    parser.add_argument("--orbit-table-out", type=Path, help="write joined quotient rotation orbit table JSON")
