#!/usr/bin/env python3
"""Exact shared-predecessor interaction search for d=5 witnesses."""
...
from torus_nd_d5_shared_pred_interaction_common import (
    ...
    target.append(
        {
            ...
            "search_metrics": {
                ...
                "universal_m_cycle": item["universal_m_cycle"],
            },
        }
    )
...
def _regime_counts(rows: Sequence[Dict[str, object]]) -> Dict[str, int]:
    ...
    return {
        ...
        "both_count": sum(1 for item in clean_strict if item["overall_kind"] == "both"),
        "cycle_only_count": sum(1 for item in clean_strict if item["overall_kind"] == "cycle_only"),
        "monodromy_only_count": sum(1 for item in clean_strict if item["overall_kind"] == "monodromy_only"),
        "trivial_count": sum(1 for item in clean_strict if item["overall_kind"] == "trivial"),
        "improved_mixed_count": sum(1 for item in clean_strict if item["improves_mixed_baseline"]),
        "universal_m_cycle_count": sum(1 for item in clean_strict if item["universal_m_cycle"]),
    }
        ...
        out.append(
            {
                ...
                "best_mixed_rule": _best_rows(items, "both", limit=1)[0] if any(
                    item["clean_all"] and item["strict_all"] and item["overall_kind"] == "both"
                    for item in items
                ) else None,
            }
        )
...
def _flag_key(rule_payload: Dict[str, object]) -> Tuple[object, ...]:
    return (rule_payload["predecessor_flag_name"],)


def _layer2_pair_key(rule_payload: Dict[str, object]) -> Tuple[object, ...]:
    return (
...
def _layer3_gadget_key(rule_payload: Dict[str, object]) -> Tuple[object, ...]:
    return (
...
def _evaluate_stage(
    ...
    *,
    ...
    pre_by_bit = {
        bit_name: {m: precompute_m(m, bit_name) for m in m_values}
        for bit_name in bit_set
    }
    out = []
    progress = None
    task = None
...
def _candidate_rows(stage1_rows: Sequence[Dict[str, object]], stage2_rows: Sequence[Dict[str, object]] | None) -> List[Dict[str, object]]:
    ...
    clean_rows = sorted(
        [item for item in list(stage1_rows) + list(stage2_rows or []) if item["clean_all"]],
...
def _all_clean_universal(rows: Sequence[Dict[str, object]]) -> bool:
    clean_strict = [item for item in rows if item["clean_all"] and item["strict_all"]]
    return bool(clean_strict) and all(item["universal_m_cycle"] for item in clean_strict)
    ...
    *,
    ...
    summary = {
        ...
        "stage1": {
            ...
            "all_clean_strict_universal_m_cycle": _all_clean_universal(stage1_results),
            "flag_summaries": _group_counts(stage1_results, _flag_key),
            "layer2_pair_summaries": _group_counts(stage1_results, _layer2_pair_key),
            "layer3_gadget_summaries": _group_counts(stage1_results, _layer3_gadget_key),
        ...
        "recommended_next_step": (
            "If the shared-switch family still fails to beat the mixed baseline, move to a broader genuinely interactive layer-2/layer-3 family or formalize the separability no-go for the present predecessor-switch branch."
        ),
    }
    if stage2_results is not None:
        summary["stage2"] = {
            ...
            "all_clean_strict_universal_m_cycle": _all_clean_universal(stage2_results),
            "flag_summaries": _group_counts(stage2_results, _flag_key),
            "layer2_pair_summaries": _group_counts(stage2_results, _layer2_pair_key),
            "layer3_gadget_summaries": _group_counts(stage2_results, _layer3_gadget_key),
...
def _print_summary(summary: Dict[str, object], *, use_rich: bool) -> None:
    lines = [
        ...
        (
            ...
            f"universal={summary['stage1']['counts']['universal_m_cycle_count']}"
        ),
    ]
    if summary["stage2"] is not None:
        lines.append(
            (
                ...
                f"universal={summary['stage2']['counts']['universal_m_cycle_count']}"
            )
        )
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Search the d=5 shared-predecessor interaction family.")
