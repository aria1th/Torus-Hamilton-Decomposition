...
within the actual Case~II scaffold the remaining boundary corrections alone do not repair this obstruction, so an additional algebraic correction performs genuine bulk repair.
...
\begin{proposition}[deleting the added Case~II family is already noninjective]\label{prop:boundary-no-go}
Assume $m=6q+4\ge 10$.
...
So within the actual Case~II scaffold, the added Case~II affine family performs genuine bulk repair work that the remaining boundary corrections do not supply.
\end{proposition}
...
\]
Thus the added Case~II affine family does more than reconnect endpoints: it changes the interior drift on even lanes from the primary $+3$ rule to $+2$, and on odd lanes from $+3$ to $+6$, thereby interleaving the three residue-$3$ strands into one cycle.
...
\begin{proof}
The structural part of the even argument has already been isolated in the main text: Proposition~\ref{prop:primary-geometry-obstruction} identifies the first obstruction in the primary geometry, and Proposition~\ref{prop:boundary-no-go} shows that the extra Case~II affine family is genuine bulk repair rather than a boundary-only patch.
It remains to pass from Definition~\ref{def:routeE} to explicit return maps and then to the cycle closure.
...
Propositions~\ref{prop:R1-caseI}, \ref{prop:R1-caseII}, \ref{prop:R0-caseI-data}, and \ref{prop:R0-caseII-data} give the explicit lane maps $T_c$ and return times $\rho_c$; in Case~II these formulas encode exactly the altered interior shifts created by the extra affine family.
Proposition~\ref{prop:routeE-splice} then packages the closure step as a finite splice permutation on arithmetic family-blocks.
...
Appendix~\ref{app:routeE} is the lookup sheet: it records the closed-form return maps $R_0,R_1,R_2$ used later in the proof.
Appendix~\ref{app:routeE-derivation} is the derivation appendix: starting from Definition~\ref{def:routeE}, it evaluates the low-layer transducer and proves Proposition~\ref{prop:routeE-return-formulas}, which is exactly the source of the formulas listed in Appendix~\ref{app:routeE}.
...
A reader mainly interested in proof flow may therefore read the appendices in the order Appendix~\ref{app:routeE}, then Appendix~\ref{app:routeE-cycle}, and finally Appendix~\ref{app:routeE-derivation}; Appendix~\ref{app:m4} and Appendix~\ref{app:verification} are archival supplements for the finite witness and the supplementary checks.
...
For orientation, the appendix may be read in the following order:
\begin{enumerate}[label=\textup{(\roman*)},leftmargin=2.5em]
\item Proposition~\ref{prop:R1-caseI}: color~$1$, Case~I $(m\equiv 0,2\pmod 6)$;
\item Proposition~\ref{prop:R1-caseII}: color~$1$, Case~II $(m\equiv 4\pmod 6)$;
\item Proposition~\ref{prop:R0-caseI-data}: color~$0$, Case~I;
\item Proposition~\ref{prop:R0-caseII-data}: color~$0$, Case~II;
\item Proposition~\ref{prop:routeE-splice}: the common splice-permutation closure for the remaining lane maps.
\end{enumerate}
...
Relative to Case~I, the only new long defect component is the extra $R$-branch on the affine line $x\equiv 1+2y$.
The easy lanes are the genuinely generic ones: after the initial $B$-stall at $y=0$, an even lane meets one $R$-point before the $A$-point, while an odd lane meets the $A$-point first and then the upper $R$-point.
The six terminal lanes $x\in\{m-6,m-5,m-4,m-3,m-2,m-1\}$ are the only splice lanes where the isolated points $Q,S$ or the top $R$-hit alter this pattern, so Proposition~\ref{prop:R0-caseII-data} is easiest to read in exactly that order.
...
\smallskip
\noindent\textbf{Case $3\le x\le m-8$ (generic lanes).}
...
For archival completeness, we also record the three color orbits obtained by iterating from $(0,0,0)$; each list closes by returning to its first vertex:
\begingroup
\footnotesize\ttfamily
\noindent\textbf{Color 0: }(0,0,0) -> (0,0,1) -> (1,0,1) -> (1,0,2) -> (1,1,2) -> (1,1,3) -> (2,1,3) -> (2,1,0) -> (3,1,0) -> (3,1,1) -> (3,1,2) -> (3,2,2) -> (3,2,3) -> (3,2,0) -> (3,2,1) -> (0,2,1) ->\\
(1,2,1) -> (1,2,2) -> (1,2,3) -> (1,3,3) -> (1,3,0) -> (1,3,1) -> (1,3,2) -> (2,3,2) -> (3,3,2) -> (3,3,3) -> (3,3,0) -> (3,3,1) -> (3,0,1) -> (3,0,2) -> (0,0,2) -> (0,1,2) ->\\
(0,2,2) -> (0,2,3) -> (0,2,0) -> (0,3,0) -> (0,3,1) -> (0,3,2) -> (0,3,3) -> (0,0,3) -> (1,0,3) -> (1,0,0) -> (1,1,0) -> (1,2,0) -> (2,2,0) -> (2,2,1) -> (2,3,1) -> (2,0,1) ->\\
(2,0,2) -> (2,0,3) -> (3,0,3) -> (3,1,3) -> (0,1,3) -> (0,1,0) -> (0,1,1) -> (1,1,1) -> (2,1,1) -> (2,1,2) -> (2,2,2) -> (2,2,3) -> (2,3,3) -> (2,3,0) -> (2,0,0) -> (3,0,0) ->\\
(0,0,0).\par\medskip
\noindent\textbf{Color 1: }(0,0,0) -> (0,1,0) -> (1,1,0) -> (2,1,0) -> (2,2,0) -> (2,3,0) -> (3,3,0) -> (0,3,0) -> (1,3,0) -> (1,0,0) -> (1,0,1) -> (1,1,1) -> (1,1,2) -> (2,1,2) -> (2,1,3) -> (2,2,3) ->\\
(3,2,3) -> (3,3,3) -> (3,0,3) -> (3,0,0) -> (3,0,1) -> (0,0,1) -> (0,1,1) -> (0,1,2) -> (0,1,3) -> (0,2,3) -> (0,3,3) -> (1,3,3) -> (2,3,3) -> (2,0,3) -> (2,0,0) -> (2,0,1) ->\\
(2,1,1) -> (3,1,1) -> (3,2,1) -> (3,2,2) -> (0,2,2) -> (1,2,2) -> (1,3,2) -> (1,0,2) -> (1,0,3) -> (1,1,3) -> (1,2,3) -> (1,2,0) -> (1,2,1) -> (2,2,1) -> (2,2,2) -> (2,3,2) ->\\
(2,0,2) -> (3,0,2) -> (3,1,2) -> (3,1,3) -> (3,1,0) -> (3,2,0) -> (0,2,0) -> (0,2,1) -> (0,3,1) -> (1,3,1) -> (2,3,1) -> (3,3,1) -> (3,3,2) -> (0,3,2) -> (0,0,2) -> (0,0,3) ->\\
(0,0,0).\par\medskip
\noindent\textbf{Color 2: }(0,0,0) -> (1,0,0) -> (2,0,0) -> (2,1,0) -> (2,1,1) -> (2,2,1) -> (3,2,1) -> (3,3,1) -> (0,3,1) -> (0,0,1) -> (0,0,2) -> (1,0,2) -> (2,0,2) -> (2,1,2) -> (3,1,2) -> (0,1,2) ->\\
(1,1,2) -> (1,2,2) -> (2,2,2) -> (3,2,2) -> (3,3,2) -> (3,0,2) -> (3,0,3) -> (0,0,3) -> (0,1,3) -> (1,1,3) -> (1,1,0) -> (1,1,1) -> (1,2,1) -> (1,3,1) -> (1,0,1) -> (2,0,1) ->\\
(3,0,1) -> (3,1,1) -> (0,1,1) -> (0,2,1) -> (0,2,2) -> (0,3,2) -> (1,3,2) -> (1,3,3) -> (1,0,3) -> (2,0,3) -> (2,1,3) -> (3,1,3) -> (3,2,3) -> (0,2,3) -> (1,2,3) -> (2,2,3) ->\\
(2,2,0) -> (3,2,0) -> (3,3,0) -> (3,0,0) -> (3,1,0) -> (0,1,0) -> (0,2,0) -> (1,2,0) -> (1,3,0) -> (2,3,0) -> (2,3,1) -> (2,3,2) -> (2,3,3) -> (3,3,3) -> (0,3,3) -> (0,3,0) ->\\
(0,0,0).\par
\endgroup
