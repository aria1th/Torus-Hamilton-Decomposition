...
The proof is organized around return maps.
Because every legal step increases the layer function $S=i+j+k$ by $1$, Hamiltonicity on $V$ reduces to the dynamics of an $m$-step return map on the plane $P_0=\{S=0\}$.
For odd $m$, five Kempe swaps from the canonical coloring produce a valid coloring whose three return maps on $P_0$ are affine-conjugate to the standard two-dimensional $m^2$-odometer.
For even $m$, a sign-product invariant rules out every Kempe-from-canonical approach.
We therefore construct an explicit low-layer direction assignment, Route~E, supported on the three lowest layers.
In adapted bulk coordinates its return maps are finite-defect perturbations of the same odometer mechanism; a first-return reduction to one-dimensional lane maps, together with a finite splice analysis, then closes the proof.
...
These results concern a single Hamilton cycle.
On the decomposition side, Keating studied $2$-ply Hamiltonicity for abelian Cayley digraphs with two generators; in particular, this determines when the Cartesian product of two directed cycles decomposes into two directed Hamilton cycles~\cite{Keating1985}.
Bogdanowicz later treated decompositions into directed cycles of a prescribed common length~\cite{Bogdanowicz2017} and identified Hamilton cycles (and, in certain two-factor cases, two arc-disjoint Hamilton cycles) under explicit arithmetic hypotheses~\cite{Bogdanowicz2020}.
By contrast, Darijani, Miraftab, and Morris study arc-disjoint Hamilton \emph{paths}: they prove the two-factor and $r\ge 4$ cases and leave the three-factor product as the remaining boundary in that setting~\cite{DarijaniMiraftabMorris2022}.
...
The symmetric three-dimensional torus $D(m)$ therefore lies at a natural boundary between existing Hamiltonicity results, two-factor Hamilton decompositions, and the current three-factor boundary for arc-disjoint Hamilton paths.
The present paper resolves the directed Hamilton-decomposition problem for the family $D(m)$.
...
For even $m$, the parity obstruction forces a separate explicit direction assignment, Route~E, supported on the three lowest layers.
Section~\ref{sec:even} isolates the primary geometry and the first obstruction; the appendices then carry the closed-form return-map derivation and the remaining first-return bookkeeping.
...
From the proof-theoretic point of view, the central object is not the coloring on $V$ itself but the return map on $P_0$.
The layer function provides a built-in clock of period $m$; passing to the first return strips away this uniform drift and leaves a two-dimensional dynamical system on a set of size $m^2$.
The model example is the standard odometer
\[
O(u,v)=(u+1,\ v+\mathbf 1_{u=0}),
\]
whose first coordinate is the clock and whose second coordinate records the carry when that clock wraps.
The odd case lands exactly on this model after an affine change of coordinates.
Route~E is designed so that, after one further first-return reduction to lane transversals, the even case differs from the same carry mechanism only at finitely many explicitly controlled stalls.
This is why the odometer viewpoint organizes the whole proof.
...
The appendices are part of the written proof: Appendix~\ref{app:routeE-derivation} derives the displayed Route~E return maps directly from Definition~\ref{def:routeE}, and Appendix~\ref{app:routeE-cycle} turns those formulas into single-cycle statements on $P_0$. By contrast, the supplementary artifact bundle recorded in Appendix~\ref{app:verification} is not used as a proof step; it serves only as an independent audit of the displayed formulas and of the finite $m=4$ witness.
...
Lemma~\ref{lem:return} is the global Poincar\'e-section reduction used throughout the paper.
The point of passing to $P_0$ is not only that it cuts the state space from size $m^3$ to size $m^2$.
More importantly, it removes the trivial layer drift and exposes the genuine carry dynamics of the coloring.
The standard model for that reduced dynamics is the two-dimensional odometer introduced next.
\end{remark}
...
The odometer is the canonical ``clock plus carry'' mechanism.
Its first coordinate advances uniformly, while the second changes only when the first wraps.
The odd proof will show that the return maps are literally affine copies of $O$.
The even proof is then best read as a repair problem: rebuild the same carry mechanism after a further first-return reduction, allowing only finitely many controlled deviations from the pure odometer motion.
\end{remark}
...
\begin{proposition}[Odd return maps are affine odometers]\label{prop:odd-odometer}
...
\begin{theorem}[Odd case via odometer conjugacy]\label{thm:odd-hamilton}
...
\begin{remark}
In the odometer picture, the failure is confined to a single color because only color~$2$ loses a primitive clock.
When $m$ is even, the update $k\mapsto k-2$ no longer runs through all residues of $\Zm$; it splits into two parity classes before any carry can connect them.
...
The odd proof showed more than Hamiltonicity: after factoring out the layer drift, the return maps on $P_0$ are affine copies of the standard two-dimensional odometer.
The parity barrier of Corollary~\ref{cor:parity-barrier} rules out producing that picture by Kempe swaps when $m$ is even, so we adopt a direct low-layer construction.
The design problem is to rebuild the same clock-and-carry mechanism in a new way.
We begin with the linear criterion that isolates the clock part of the return dynamics on $P_0\cong (\mathbb Z_m)^2$.
...
For the affine odd construction, Lemma~\ref{lem:skew-product} recovers the odometer picture in linear form: the second coordinate is the clock, and the first records the accumulated carry.
When $m$ is even, color $2$ loses that clock because the step $-2$ is not a unit.
So the even design problem is not to search blindly for a new coloring, but to repair the low layers until a primitive clock and its associated carry mechanism reappear, while keeping the exceptional set finite and geometrically explicit.
...
\begin{remark}
Lemma~\ref{lem:counting} is a discrete analogue of the Poincar\'e first-return map in dynamical systems: it reduces the global cycle structure of~$F$ to the dynamics of the induced map on a cross-section.
In the odd case the section $P_0$ already reveals the full two-dimensional odometer, so no further reduction is needed.
In the even case we apply the same philosophy twice, first on $P_0$ and then on explicit lane transversals, until only the one-dimensional carry map remains.
\end{remark}
...
In each coordinate system the generic step (also called the \emph{bulk} step, denoted $G$ in the orbit displays) advances one coordinate (the \emph{clock}) uniformly while preserving the other (the \emph{lane}).
This is the odometer picture again: the clock coordinate is the part that turns every time, while the lane coordinate records the slow carry created by the rare non-generic events.
Those finitely many non-generic branches are the \emph{stalls} introduced above: at a stall the lane shifts by a bounded amount, and the clock may gain or lose an extra unit.
...
Both the odd and even constructions are best read as proofs about a low-dimensional odometer hidden inside the torus.
The layer function provides the clock.
In the odd case the return map on $P_0$ is literally affine-conjugate to the standard two-dimensional odometer.
In the even case Route~E restores the same carry mechanism only after a further first-return reduction to lanes, at the price of finitely many splices.
The boundary case $m=4$ is the small exceptional instance where that low-layer surgery is recorded by a finite table rather than by a uniform arithmetic family.
...
First, can the even-case deductive surgery be formalized as a clean Route~E-specific critical-lane theorem that reads off the repaired arithmetic family-blocks and the splice permutation directly from the affine defect heights, avoiding most of the remaining itinerary bookkeeping?
...
The present proof relies heavily on a one-dimensional transversal, finitely many affine defect families, and a finite splice on arithmetic family-blocks.
In higher dimensions one would expect higher-codimension defect arrangements, multivariable congruence constraints, and a more complicated splice topology.
In particular, does Corollary~\ref{cor:parity-barrier} generalize to higher dimensions, and if so, what sort of obstruction-breaking construction can survive those additional geometric and arithmetic barriers?

\appendix

\paragraph{How to read the appendices.}
Appendix~\ref{app:routeE} is the lookup sheet: it records the closed-form return maps $R_0,R_1,R_2$.
Appendix~\ref{app:routeE-derivation} is the derivation appendix: starting from Definition~\ref{def:routeE}, it evaluates the low-layer transducer and proves Proposition~\ref{prop:routeE-return-formulas}, the source of the formulas listed in Appendix~\ref{app:routeE}.
Appendix~\ref{app:routeE-cycle} is the cycle-structure appendix: taking those formulas as input, it derives the first-return maps and return-time sums for colors~$1$ and~$0$, and then packages the final closure step in Proposition~\ref{prop:routeE-splice}.
...
Appendix~\ref{app:m4} and Appendix~\ref{app:verification} are archival supplements for the finite witness and the supplementary checks.
