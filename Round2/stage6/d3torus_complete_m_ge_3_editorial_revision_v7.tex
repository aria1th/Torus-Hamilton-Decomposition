...
When $m\equiv 4\pmod 6$, color $1$ remains trapped in three residue-$3$ strands;
within the actual Case~II scaffold the remaining boundary corrections alone do not repair this obstruction, so the Case~II repair family supplies the needed bulk repair.
...
Hence for every $v\in P_1$ the outgoing arcs are still canonical:
\[
f_0(v)=\mathrm{bump}_i(v),\qquad f_1(v)=\mathrm{bump}_j(v),\qquad f_2(v)=\mathrm{bump}_k(v).
\]
...
Hence $f_1^{-1}$ on the image of $P_1$ in $P_2$ depends only on the outgoing color-$1$ arcs from $P_1$, which are still the canonical $j$-arcs.
...
\]
with all arithmetic modulo $m$.
For example, the color-$0$ formula comes from exactly one low-layer deviation per $m$-step return.
Starting from $(i,k)\in P_0$, the first step on layer $S=0$ is in direction $0$ when $k=0$ and in direction $1$ otherwise.
The next step is always the unique layer-$1$ step, which is in direction $2$.
All remaining $m-2$ steps occur on layers $S\ge 2$ and therefore use the canonical direction $0$.
Hence $F_0(i,0)$ gains $(m-1)$ units in the $i$-coordinate and one unit in the $k$-coordinate, while $F_0(i,k)$ for $k\neq 0$ gains $(m-2)$ units in the $i$-coordinate and again one unit in the $k$-coordinate; this is exactly the displayed formula for $F_0$.
The formulas for $F_1$ and $F_2$ are obtained by the same one-cycle count.
...
\end{enumerate}
In proof-dependency form, the even case runs
\[
\text{Definition~\ref{def:routeE}}
\to \text{Proposition~\ref{prop:routeE-transducer} + Appendix~\ref{app:routeE-derivation}}
\to \text{Lemmas~\ref{lem:routeE-bulkcoords}--\ref{lem:routeE-itinerary}}
\to \text{first-return formulas}
\to \text{Proposition~\ref{prop:routeE-splice}}
\to \text{Corollaries~\ref{cor:R1-all},\ref{cor:R0-all}}
\to \text{Theorem~\ref{thm:routeE-even-main}}.
\]
The main text proves the color-$2$ branch and the obstruction/repair mechanism; Appendix~\ref{app:routeE-derivation} supplies the closed return-map derivations, and Appendix~\ref{app:routeE-cycle} carries the remaining first-return and splice bookkeeping for colors~$1$ and~$0$.
...
\begin{example}[Color $0$ in Case~II at $m=10$]\label{ex:m10-color0}
For the same value $m=10$, Proposition~\ref{prop:R0-caseII-data} gives the lane map
\[
0\mapsto 8\mapsto 5\mapsto 9\mapsto 1\mapsto 4\mapsto 3\mapsto 7\mapsto 2\mapsto 6\mapsto 0.
\]
The point is not merely the specific $10$-cycle, but its structure.
Away from finitely many splice lanes, color~$0$ follows the bulk rule $x\mapsto x+4$, so the lanes first organize into residue-$4$ arithmetic strands.
At $m=10$ these strands are
\[
(0,8,5,9,1),\qquad (4),\qquad (3,7),\qquad (2,6),
\]
which are exactly the blocks $F_{0,1},F_{0,2},F_{0,3},F_{0,4}$ from Proposition~\ref{prop:routeE-splice}.
The special terminal images
\[
1\mapsto 4,\qquad 4\mapsto 3,\qquad 7\mapsto 2,\qquad 6\mapsto 0
\]
then splice those four residue-$4$ strands cyclically into one orbit.
For general $m\equiv 10\pmod{12}$ the same residue-$4$ picture persists, and the case $m\equiv 4\pmod{12}$ differs only by a cyclic reordering of which strand contains the terminal lane $1$.
\end{example}
...
Proposition~\ref{prop:routeE-splice} packages the remaining closure step as a finite splice permutation on arithmetic family-blocks.
...
Both the odd and even constructions ultimately reduce to return-map dynamics on $P_0$; the boundary case $m=4$ is settled by the finite witness of Appendix~\ref{app:m4}.
...
The present proof should not be read as evidence that the same mechanism extends cleanly beyond the three-factor setting: it relies heavily on a one-dimensional transversal, finitely many affine defect families, and a finite splice on arithmetic family-blocks.
...
All assertions used in the proof are proved in the written text; the scripts below only re-check the displayed formulas and the finite witness.
...
\newblock in {\em Cycles in Graphs}, Annals of Discrete Mathematics 27 (1985), 81--87.
