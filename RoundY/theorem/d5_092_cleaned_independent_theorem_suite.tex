...
q \equiv u-\rho+1 \pmod m.
\]
This is the phase-$1$ source-residue invariant later packaged in $098$: it
holds at the first
...
\section{Support theorems in cleaned form}
...
\begin{proposition}[tail-length reduction; compactly reproved support theorem]
...
\begin{proposition}[chart/interface theorem; compactly reproved support theorem]
...
\begin{proposition}[regular continuation theorem; compactly reproved support theorem]
...
earliest files. The only theorem layer still imported in accepted-support form
is:
\begin{enumerate}[label=\textup{(\arabic*)},leftmargin=2.5em]
...
are now supplied by the cleanup notes $095$--$098$. So this is very close to a
self-contained theorem suite, but it is not yet the final compact bridge
replacement for every support theorem.
\end{remark}
