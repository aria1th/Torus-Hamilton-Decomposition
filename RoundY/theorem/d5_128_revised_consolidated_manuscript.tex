...
Revised consolidated manuscript after the corrected M2/M3 package and the local-provenance no-go}
\author{}
\date{March 17, 2026}
...
branch is closed on $\SelStar$; and $M6$ remains closed. The remaining work is
therefore concentrated in two places: the channel reduction $M1$ and the
construction of a full colorwise graph package for the remaining colors.
...
\midrule
$M1$ & open & channel reduction from the full odd-$m$ D5 problem to the best-seed channel $R1 \to H_{L1}$ \\
...
What remains open is therefore narrower than in the older manuscript: a channel
reduction theorem, and the missing graph-level colorwise exactness and
Hamiltonicity packages for the remaining colors.
...
\begin{placeholder}[Channel reduction / $M1$]
\label{ph:M1}
Prove a theorem-level channel classification reducing the full odd-$m$, $d=5$
problem to the best-seed channel
\[
R1\to H_{L1},
\]
or prove that all other channels are already closed and that this channel is the
...
Placeholders~\ref{ph:M1}, \ref{ph:colorwise-graph}, and
\ref{ph:remaining-M5} have been resolved.
...
\begin{proof}
Placeholder~\ref{ph:M1} supplies the channel reduction package.
...
\item \textbf{Attack $M1$ directly.}
      This is now the only theorem-side placeholder still blocking a fully
      honest statement of the reduction spine.
...
remaining open work sits entirely in $M1$ and in the graph-side colorwise
package for the remaining colors.
