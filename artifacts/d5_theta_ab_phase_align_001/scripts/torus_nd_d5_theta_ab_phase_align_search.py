#!/usr/bin/env python3
"""Free-anchor search for d=5 cyclic-equivariant master fields on Theta_AB_plus_phase_align."""
...
class _SolutionCollector(cp_model.CpSolverSolutionCallback):
    ...
    def on_solution_callback(self) -> None:
        anchor_values = {
            state_id: next(value for value in range(DIM) if self.Value(self.anchor_vars[(state_id, value)]))
...
def _solve_schema(
    ...
    *,
    ...
    model, schema, states, state_space_key, _anchor_vars, preference_expr, pattern_counts = _build_anchor_model(
        schema_name, pilot_m_values, stability_m_values
    )
    ...
    payload = {
        ...
        "rotation_orbit_count": len(rotation_orbits(states)),
        "rotation_orbit_size_histogram": rotation_orbit_histogram(states),
        "state_counts_by_m": {str(m): count for m, count in schema_state_counts_by_m(schema_name, state_space_key)},
    ...
    *,
    ...
    return {
        ...
        "search_parameters": {
            ...
            "pilot_m_values": list(pilot_m_values),
            "stability_m_values": list(stability_m_values),
            "state_space_key": list(state_space_key),
            "max_time_sec": max_time_sec,
            "workers": workers,
            "random_seed": random_seed,
            "solution_limit": solution_limit,
...
def _print_summary(summary: Dict[str, object], *, use_rich: bool) -> None:
    ...
    for item in summary["schema_results"]:
        line = (
            ...
            f"orbits={item['rotation_orbit_count']} patterns={item['pattern_counts']} "
            f"pref={item['optimal_preference_score']} runtime={item['runtime_sec']:.3f}"
        )
        if item.get("feasible_fields"):
            first_m = str(summary["search_parameters"]["pilot_m_values"][0])
            first = item["feasible_fields"][0]["pilot_validation"][first_m]
            line += (
                f" | feasible={item['feasible_field_count']} latin={first['latin']['incoming_latin']} "
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Search free-anchor d=5 master fields on Theta_AB_plus_phase_align.")
    parser.add_argument("--schemas", default="joined_anchor_five_atom_phase_align", help="comma-separated schema names")
    ...
    parser.add_argument("--max-time-sec", type=float, default=60.0, help="per-schema time budget")
    ...
    parser.add_argument("--solution-limit", type=int, default=5, help="max solutions to save from each enumeration pass")
    parser.add_argument("--out", type=Path, required=True, help="write JSON summary to this path")
    parser.add_argument("--state-table-out", type=Path, help="write quotient state table JSON")
    parser.add_argument("--orbit-table-out", type=Path, help="write rotation orbit table JSON")
    ...
    if args.state_table_out is not None or args.orbit_table_out is not None:
        ...
        for schema_name in schema_names:
            ...
            if args.state_table_out is not None:
                payload = state_table_payload(schema, states)
                if len(schema_names) == 1:
                    ...
                    path = args.state_table_out.parent / f"{schema_name}_{args.state_table_out.name}"
                    path.parent.mkdir(parents=True, exist_ok=True)
                    path.write_text(json.dumps(payload, indent=2))
            if args.orbit_table_out is not None:
                payload = orbit_table_payload(schema, states)
                if len(schema_names) == 1:
                    args.orbit_table_out.parent.mkdir(parents=True, exist_ok=True)
                    args.orbit_table_out.write_text(json.dumps(payload, indent=2))
                else:
                    path = args.orbit_table_out.parent / f"{schema_name}_{args.orbit_table_out.name}"
                    path.parent.mkdir(parents=True, exist_ok=True)
                    path.write_text(json.dumps(payload, indent=2))
