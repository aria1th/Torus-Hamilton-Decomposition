#!/usr/bin/env python3
"""Validate saved free-anchor d=5 master-field candidates."""
...
def run_validation(search_summary: Dict[str, object], *, m_values: Sequence[int], field_limit: int) -> Dict[str, object]:
    ...
    return {
        "task_id": "D5-MASTER-FIELD-FREE-ANCHOR-001",
...
def _print_summary(summary: Dict[str, object], *, use_rich: bool) -> None:
    ...
    for item in summary["validated_fields"][:5]:
        ...
        lines.append(
            f"{item['schema']} field#{item['field_index']}: m={first['m']} latin={first['latin']['incoming_latin']} "
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Validate free-anchor master-field candidates.")
    parser.add_argument("--search-summary-json", type=Path, required=True, help="free-anchor search summary JSON")
