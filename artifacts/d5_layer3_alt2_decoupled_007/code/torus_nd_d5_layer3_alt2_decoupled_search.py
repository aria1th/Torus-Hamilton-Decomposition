#!/usr/bin/env python3
"""Exact search for the decoupled two-old-bit family with layer-3 alt {0,2}."""
        ...
        for layer3_bit_name in OLD_BIT_NAMES:
            for layer2_alt in LAYER2_ALTERNATES:
                for layer2_table in ((2, layer2_alt), (layer2_alt, 2)):
                    for layer3_alt in LAYER3_ALTERNATES:
                        for layer3_table in ((3, layer3_alt), (layer3_alt, 3)):
                            rows.append(
                                Rule(
...
def run_search(*, m_values: Sequence[int], use_rich: bool) -> Dict[str, object]:
    ...
    summary = {
        ...
        "search_space": {
            ...
            "layer3_alternates": list(LAYER3_ALTERNATES),
            "layer2_orientations": ["2/a", "a/2"],
            "layer3_orientations": ["3/b", "b/3"],
        ...
        "recommended_next_step": (
            "If no combined witness exists, add one predecessor-tail flag to layer 3 while keeping a successful layer-2 cycle split fixed."
        ),
    }
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Search the decoupled two-old-bit d=5 family with layer-3 alt {0,2}.")
